...
namespace JC.BluetoothUnity.Demo
{
    public class MessagePanel : MonoBehaviour
    {
        ...
        void Update()
        {
            int available = BluetoothManager.Instance.Available();
            if(available > 0)
            {
                string line = BluetoothManager.Instance.ReadLine();
                PushMessage($"<color=lime>REMOTE</color>: {line}\n");
            }
            _isConnectedText.text = $"IsConnected: {BluetoothManager.Instance.IsConnected()} ({BluetoothManager.Instance.GetConnectedDevice()})";
        }
        ...
        {            
            ...
            if(BluetoothManager.Instance.Send(_sendInput.text + "\r\n"))
            {
                // send ok
                _sendInput.text = "";
            }
